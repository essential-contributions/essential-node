...
fn pre_new_pred_exists() -> Access<'static> {
    test_access!({
...
fn post_new_pred_does_not_exist() -> Access<'static> {
    test_access!({
        let mut access = setup_access();
        access.post[1][0] = 0;
        access.post[1][2] = 0;
...
fn pre_existing_pred_does_not_exist() -> Access<'static> {
    test_access!({
        let mut access = setup_access();
        access.pre[1][1] = 0;
        access
...
fn post_existing_pred_does_not_exist() -> Access<'static> {
    test_access!({
        let mut access = setup_access();
        access.post[1][1] = 0;
        access
...
fn test_found_predicates() {
    ...
    exec(
        ...
        &mut repeat,
        &ops,
        post_existing_pred_does_not_exist(),
    )
    .unwrap();
    ...
    exec(
        ...
        &mut repeat,
        &ops,
        pre_existing_pred_does_not_exist(),
    )
    .unwrap();
